...
export class InventoryService {
   ...
   */
  async createItem(input: CreateInventoryItemInput, user: User): Promise<Stock> {
    return this.dataSource.transaction(async (manager: EntityManager) => {
      try {
        this.logger.log(`Creating inventory item ${input.product.name} for cafe ${input.cafeId}`);
        ...
        const existingItem = await manager.findOne(Stock, {
        ...
        const inventoryItem = manager.create(Stock, {
          ...input,
          status: InventoryStatus.ACTIVE,
          lastRestocked: new Date(),
        });

        const savedItem = await manager.save(Stock, inventoryItem);

        // Check for immediate alerts
        const alerts = this.generateStockAlerts([savedItem]);
        if (alerts.length > 0) {
          this.logger.warn(`Stock alerts for new item ${savedItem.product.name}:`, alerts);
        }

        this.logger.log(`Stock item ${savedItem.product.name} created successfully`);
        return savedItem;
   ...
   */
  async updateItem(id: string, input: UpdateInventoryItemInput, user: User): Promise<Stock> {
    return this.dataSource.transaction(async (manager: EntityManager) => {
      const item = await manager.findOne(Stock, { where: { id } });
      ...
      await manager.update(Stock, id, input);

      const updatedItem = await manager.findOne(Stock, { where: { id } });

      this.logger.log(`Stock item ${item.product.name} updated successfully`);
      return updatedItem!;
    });
  }
   ...
   */
  async updateStock(id: string, input: UpdateInventoryStockInput, user: User): Promise<Stock> {
    return this.dataSource.transaction(async (manager: EntityManager) => {
      const item = await manager.findOne(Stock, { where: { id } });
      ...
      switch (input.operation) {
        ...
        case 'SUBTRACT':
          ...
          if (item.currentStock - input.quantity < 0) {
            this.logger.warn(`Stock deduction would result in negative stock for ${item.product.name}. Setting to 0.`);
          }
          break;
        case 'SET':
      ...
      await manager.update(Stock, id, updates);
      ...
      if (alerts.length > 0) {
        this.logger.warn(`Stock alerts for ${item.product.name}:`, alerts);
        // Here you could emit events for real-time notifications
      }

      this.logger.log(`Stock updated for ${item.product.name}: ${item.currentStock} -> ${newStock} (${input.operation} ${input.quantity})`);
      return updatedItem!;
    });
  }
   ...
   */
  async batchUpdateStock(updates: BatchStockUpdate[], user: User): Promise<Stock[]> {
    return this.dataSource.transaction(async (manager: EntityManager) => {
      const results: Stock[] = []
      const alertItems: Stock[] = []

      for (const update of updates) {
        const item = await manager.findOne(Stock, { where: { id: update.itemId } });
        ...
        const stockUpdates: Partial<Stock> = { currentStock: newStock }

        if (update.operation === 'ADD' && update.quantity > 0) {
          stockUpdates.lastRestocked = new Date()
        }

        await manager.update(Stock, update.itemId, stockUpdates);

        const updatedItem = await manager.findOne(Stock, { where: { id: update.itemId } });
        if (updatedItem) {
   ...
   */
  async deleteItem(id: string, user: User): Promise<Stock> {
    ...
    this.logger.log(`Stock item ${item.product.name} deleted successfully`);
    return item;
  }
  ...
  async findByCafe(cafeId: string, options: InventoryServiceOptions): Promise<PaginatedInventoryResponse> {
    ...
    if (sort?.field && sort?.direction) {
      ...
      queryBuilder.orderBy('inventory.product.name', 'ASC');
    }
   ...
   */
  async findLowStockItems(cafeId: string, user: User): Promise<Stock[]> {
   ...
   */
  async findOutOfStockItems(cafeId: string, user: User): Promise<Stock[]> {
    return this.inventoryRepository
      .createQueryBuilder('inventory')
      .where('inventory.cafeId = :cafeId', { cafeId })
      .andWhere('inventory.currentStock = 0')
   ...
   */
  async findExpiringItems(cafeId: string, days: number, user: User): Promise<Stock[]> {
   ...
   */
  async search(cafeId: string, query: string, options: InventoryServiceOptions): Promise<Stock[]> {
  ...
  async findById(id: string, user?: User): Promise<Stock | null> {
  ...
  async deductStockForOrder(deductions: { itemId: string; quantity: number; orderId?: string }[], userId: string): Promise<void> {
    await this.dataSource.transaction(async (manager: EntityManager) => {
      for (const deduction of deductions) {
        const item = await manager.findOne(Stock, { where: { id: deduction.itemId } });

        if (!item) {
          this.logger.warn(`Stock item ${deduction.itemId} not found for deduction`);
          continue;
        }

        if (item.currentStock >= deduction.quantity) {
          await manager.update(Stock, deduction.itemId, {
          ...
          const updatedItem = await manager.findOne(Stock, { where: { id: deduction.itemId } });
          if (updatedItem) {
            const alerts = this.generateStockAlerts([updatedItem]);
            if (alerts.length > 0) {
          ...
          this.logger.warn(`Insufficient stock for ${item.product.name}. Available: ${item.currentStock}, Required: ${deduction.quantity}`);
          ...
          await this.publishStockAlerts([{
            ...
            item,
            severity: 'HIGH',
            message: `Critical: Cannot fulfill order for ${item.product.name}. Available: ${item.currentStock}, Required: ${deduction.quantity}`,
  ...
  async generateInventoryForecast(cafeId: string, itemIds?: string[], timeHorizon: number = 30): Promise<ForecastData[]> {
    try {
      ...
      this.logger.error(`Failed to generate inventory forecast: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async calculateReorderPoints(cafeId: string): Promise<ReorderSuggestion[]> {
    try {
      ...
      this.logger.error(`Failed to calculate reorder points: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async getCostAnalytics(cafeId: string, startDate?: Date, endDate?: Date): Promise<CostAnalytics> {
    try {
      ...
      this.logger.error(`Failed to get cost analytics: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async getInventoryMetrics(cafeId: string, period: number = 30): Promise<InventoryMetrics> {
    try {
      ...
      this.logger.error(`Failed to get inventory metrics: ${error.message}`, error.stack);
      throw error;
    }
  }
      ...
      for (const data of inventoryData) {
        try {
          if (options.upsert && data.sku) {
            ...
            if (existingItem) {
              await manager.update(Stock, existingItem.id, data);
              const updated = await manager.findOne(Stock, { where: { id: existingItem.id } });
              if (updated) results.updated!.push(updated);
            } else {
              const newItem = manager.create(Stock, { ...data, cafeId });
              const created = await manager.save(Stock, newItem);
          ...
          results.failed++;
          results.errors.push(`SKU ${data.sku}: ${error.message}`);
        }
      }
  ...
  async bulkStockAdjustment(
    ...
    return this.dataSource.transaction(async (manager: EntityManager) => {
      for (const adjustment of adjustments) {
        try {
          const item = await manager.findOne(Stock, { where: { id: adjustment.itemId } });
          ...
          await manager.update(Stock, adjustment.itemId, {
          ...
          results.failed++;
          results.errors.push(`Item ${adjustment.itemId}: ${error.message}`);
        }
      }
  ...
  async generateComprehensiveReport(cafeId: string, user: User): Promise<{
    ...
    try {
      ...
      this.logger.error(`Failed to generate comprehensive report: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  private async validateInventoryData(data: Partial<Stock>, cafeId: string): Promise<void> {
    if (!data.sku) throw new Error('SKU is required');
    if (!data.product.name) throw new Error('Item name is required');
  ...
  private generateStockAlerts(items: Stock[]): StockAlert[] {
    ...
    for (const item of items) {
      ...
      // Low stock alert
        ...
        alerts.push({
          ...
          item,
          severity: 'MEDIUM',
          message: `${item.product.name} is running low (${item.currentStock} remaining, minimum: ${item.minimumStock})`,
      ...
      if (item.maximumStock && item.currentStock > item.minimumStock) {
        ...
        if (item.currentStock <= reorderPoint) {
          alerts.push({
            ...
            item,
            severity: 'LOW',
            message: `${item.product.name} has reached reorder point (${item.currentStock}/${reorderPoint})`,
      ...
      if (item.maximumStock && item.currentStock > item.maximumStock * 1.2) { // 20% over maximum
        alerts.push({
          ...
          item,
          severity: 'LOW',
          message: `${item.product.name} is overstocked (${item.currentStock}/${item.maximumStock} maximum)`,
      ...
      if (item.expiryDate) {
        ...
        if (daysUntilExpiry <= 0) {
          alerts.push({
            ...
            item,
            severity: 'HIGH',
            message: `${item.product.name} has expired`,
          ...
          alerts.push({
            ...
            item,
            severity: 'MEDIUM',
            message: `${item.product.name} expires in ${daysUntilExpiry} day(s)`,
          ...
          alerts.push({
            ...
            item,
            severity: 'LOW',
            message: `${item.product.name} expires in ${daysUntilExpiry} days`,
