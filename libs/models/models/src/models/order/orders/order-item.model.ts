...
import { OrderItemCustomization } from './order-item-customization.model';
import { OrderItemCounterStatus } from './order-item-counter-status.model';
...
export class OrderItem extends BaseEntity {
  ...
  // Customizations and modifications (now in separate table)
  @OneToOne(() => OrderItemCustomization, customization => customization.orderItem, { cascade: true, nullable: true })
  declare customization: Relation<OrderItemCustomization>;
  ...
  @OneToMany(() => OrderItemCounterStatus, counterStatus => counterStatus.orderItem, { cascade: true })
  ...
  get isCompleted(): boolean {
    if (!this.counterStatuses?.length) return false;
    return this.counterStatuses.every(cs => cs.status === 'completed');
  }
